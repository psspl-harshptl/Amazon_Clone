  ...
  return (
    <div className="flex min-h-screen bg-[#EAEDED]">
      ...
      <main className="flex-1 px-6 py-6 space-y-5 overflow-auto">
        ...
        <section className="bg-white border border-gray-200 rounded shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            <div className="flex items-center gap-4">
              {seller.storeLogo ? (
                <img src={seller.storeLogo} alt="store" className="w-12 h-12 rounded-full object-cover border flex-shrink-0" />
              ) : (
                <div className="w-12 h-12 rounded-full bg-[#232F3E] flex items-center justify-center text-white text-lg font-bold flex-shrink-0">
                  {seller.name?.[0]?.toUpperCase()}
                </div>
              )}
              <div>
                <h2 className="text-[16px] font-bold text-[#0F1111]">{seller.storeName || seller.name}</h2>
                <p className="text-[12px] text-[#565959]">{seller.email}</p>
            ...
            <div className="flex gap-2">
              ...
              {seller.sellerStatus !== 'rejected' && (
                <button onClick={() => setRejectModal(true)}
                  className="bg-white hover:bg-[#F7F8F8] border border-gray-300 text-[#CC0C39] text-[13px] font-medium px-4 py-1.5 rounded shadow-sm transition-colors">
                  Reject
                </button>
              )}
            </div>
          ...
          <div className="p-5 grid grid-cols-2 gap-4 text-[13px]">
            ...
            {seller.sellerRejectionReason && (
              <div className="col-span-2">
                <p className="text-[11px] text-[#565959] uppercase font-medium mb-0.5">Rejection Reason</p>
                <p className="text-[13px] text-[#CC0C39]">{seller.sellerRejectionReason}</p>
              </div>
            )}
          </div>
        </section>
        ...
        {seller.bankDetails && (
          <section className="bg-white border border-gray-200 rounded shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100">
              <h2 className="text-[15px] font-bold text-[#0F1111]">Bank Details</h2>
            </div>
            <div className="p-5 grid grid-cols-2 gap-4 text-[13px]">
              ...
              <Field label="IFSC Code" value={seller.bankDetails.ifscCode || seller.bankDetails.ifsc || '—'} />
            </div>
          </section>
        )}
        ...
        <section className="bg-white border border-gray-200 rounded shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100">
            <h2 className="text-[15px] font-bold text-[#0F1111]">Financials</h2>
          </div>
          <div className="p-5 space-y-5">
            <div className="grid grid-cols-3 gap-4">
              <StatCard label="Cleared Balance" value={fmt(clearedBalance)} />
              <StatCard label="Items Sold" value={orderStats?.totalItems || 0} />
              <StatCard label="Total Revenue" value={fmt(orderStats?.totalRevenue || 0)} />
            </div>

            {payoutRequests.length > 0 && (
              <div>
                <p className="text-[12px] font-semibold text-[#0F1111] mb-2">Recent Payout Requests</p>
        ...
        <section className="bg-white border border-gray-200 rounded shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            <h2 className="text-[15px] font-bold text-[#0F1111]">Products ({products.length})</h2>
          </div>
          {products.length === 0 ? (
            <p className="text-[13px] text-[#565959] px-5 py-8">No products listed yet</p>
          ) : (
            <table className="w-full text-[13px]">
              <thead className="bg-[#F7F8F8] border-b border-gray-200">
                <tr className="text-left text-[11px] text-[#565959] uppercase">
              ...
              <tbody className="divide-y divide-gray-50">
                {products.map(p => (
                  <tr key={p.id} className="hover:bg-[#F7F8F8]">
                    ...
                    <td className="px-5 py-3 text-[#0F1111]">{fmt(p.price)}</td>
...
function StatCard({ label, value }) {
  return (
    <div className="bg-[#F7F8F8] border border-gray-200 rounded p-4 text-center">
      ...
      <p className="text-[17px] font-bold text-[#0F1111]">{value}</p>
    </div>
  );
}
